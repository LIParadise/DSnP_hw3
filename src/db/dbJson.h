...
// helper for class DBJson;
// to determine if there's file.
class CmdParser;
...
class DBJson
{
  public:
    ...
    // helpers.
    bool find_key ( size_t&, const string& ) const;
    bool ifFile() const { return _ifFile; }
    ...
    operator void* () const { 
      if( ifFile() ){
        return reinterpret_cast<void*> ((size_t)1);
      }else{
        return nullptr;
      }
    }
